...
export function getMasterDb(): Knex {
  if (!masterDbInstance) {
    // Soportar tanto MASTER_DB_URL como componentes separados
    const connectionString = process.env.MASTER_DB_URL;
    
    if (connectionString) {
      // Usar URL completa (para Neon/producción)
      ...
      // Usar componentes separados (desarrollo local)
      masterDbInstance = knex({
        client: 'pg',
        connection: {
